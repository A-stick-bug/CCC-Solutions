...
# Maintain grid indices in a SortedList
# For each row, store the column indices with cars in a SortedList (do the opposite for columns)
#
# TC: O(n^2 * log(n))
# Note: a deque with lazy removal can be used to remove the log factor
...
rows = [SortedList() for _ in range(R)]
cols = [SortedList() for _ in range(C)]

for i in range(R):  # get cells with cars
    for j in range(C):
...
q = deque()
for i in range(R):  # get cells that are not blocked
    for j in range(C):
        if ((rows[i] and rows[i][0] == j and board[i][j] == "W") or
                (rows[i] and rows[i][-1] == j and board[i][j] == "E") or
                (cols[j] and cols[j][0] == i and board[i][j] == "N") or
                (cols[j] and cols[j][-1] == i and board[i][j] == "S")):
            add_to_queue(i, j)

while q:  # topo sort to get everything else in order
    r, c = q.popleft()
    print(f"({r},{c})")

    if rows[r] and board[r][rows[r][0]] == "W":
        add_to_queue(r, rows[r][0])
    if rows[r] and board[r][rows[r][-1]] == "E":
        add_to_queue(r, rows[r][-1])

    if cols[c] and board[cols[c][0]][c] == "N":
        add_to_queue(cols[c][0], c)
    if cols[c] and board[cols[c][-1]][c] == "S":
        add_to_queue(cols[c][-1], c)
