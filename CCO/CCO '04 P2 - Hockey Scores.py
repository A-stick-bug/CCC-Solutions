# https://dmoj.ca/problem/cco04p2
# Minimum number of increasing subsequences needed to cover all elements
# Similar to https://dmoj.ca/problem/cco24p2
#
# TC: O(nlogn)
...
def solve():
    n = int(input())
    arr = [sorted(map(int, input().strip().split("-"))) for _ in range(n)]
    arr.sort()  # make 1 dimension increasing

    piles = []
    tops = SortedList()  # (2nd dimension value, index in piles)
    for a, b in arr:
        i = tops.bisect_left((b, inf)) - 1

        if i == -1:  # new pile needed
            tops.add((b, len(piles)))
            piles.append([(a, b)])
        else:
            best_idx = tops[i][1]
            tops.remove((piles[best_idx][-1][1], best_idx))
            piles[best_idx].append((a, b))
            tops.add((piles[best_idx][-1][1], best_idx))

    print(len(piles))
    for pile in piles:
        print(" ".join(map(lambda x: "-".join(map(str, x)), pile)))
